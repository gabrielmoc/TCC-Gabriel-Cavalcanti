...
if [[ -z "${SCENARIO}" || -z "${PROFILE}" || -z "${ENDPOINT}" || -z "${RUN_NUMBER}" ]]; then
  echo "usage: tests/load/run-case2-battery.sh <baseline|redis-cache> <moderado|forte> <catalog|recommendations> <run-number>"
  exit 1
fi
...
case "${PROFILE}" in
  moderado)
    ...
    RAMP_TARGET_VUS="${RAMP_TARGET_VUS:-50}"
    RAMP_UP_DURATION="${RAMP_UP_DURATION:-20s}"
    SUSTAIN_DURATION="${SUSTAIN_DURATION:-45s}"
    RAMP_DOWN_DURATION="${RAMP_DOWN_DURATION:-15s}"
    SLEEP_SECONDS="${SLEEP_SECONDS:-0.15}"
    ;;
  forte)
    START_VUS="${START_VUS:-10}"
    RAMP_TARGET_VUS="${RAMP_TARGET_VUS:-90}"
    RAMP_UP_DURATION="${RAMP_UP_DURATION:-40s}"
    SUSTAIN_DURATION="${SUSTAIN_DURATION:-120s}"
    RAMP_DOWN_DURATION="${RAMP_DOWN_DURATION:-30s}"
    SLEEP_SECONDS="${SLEEP_SECONDS:-0.1}"
    ;;
  *)
    echo "perfil inválido: ${PROFILE}. Use moderado ou forte."
    exit 1
    ;;
esac
...
cd "${ROOT_DIR}"
node tests/load/collect-runtime-metrics.mjs --output "${RESULTS_DIR}/runtime-metrics-before.json"

BASE_URL="http://127.0.0.1:3000" \
SCENARIO_LABEL="${SCENARIO}" \
LOAD_PROFILE="case-2-${PROFILE}" \
START_VUS="${START_VUS}" \
...
node tests/load/collect-runtime-metrics.mjs --output "${RESULTS_DIR}/runtime-metrics-after.json"
